import { saveHistoryEntry, getHistory, HistoryEntry } from '../../src/services/HistoryService';
...
async function clearHistoryFile(questionType: string) {
...
async function runManualTests() {
    ...
    await clearHistoryFile(questionType1);
    await clearHistoryFile(questionType2);
    await clearHistoryFile(questionType3);
    // 也清理一下上次測試可能遺留的 9.9.9 (雖然不在此次儲存之列)
    await clearHistoryFile('9.9.9'); 

    // 範例作答數據 - 更新：使用 UUID，移除 Omit 中的 recordId
    const entryData1_1: Omit<HistoryEntry, 'timestamp'> = { // Omit 中不再需要 'recordId'
    ...
    const entryData1_2: Omit<HistoryEntry, 'timestamp'> = { // Omit 中不再需要 'recordId'
    ...
    const entryData2_1: Omit<HistoryEntry, 'timestamp'> = { // Omit 中不再需要 'recordId'
    ...
    const entryData3_1: Omit<HistoryEntry, 'timestamp'> = { 
        ...entryData1_1, // 先展開 entryData1_1
        UUID: 'manual-test-uuid-211-delta', // 然後用這個 UUID 覆蓋 entryData1_1 中的 UUID
        questionData: {...entryData1_1.questionData, passage: "Passage for type 3.1"}, // 更新 questionData
        userAnswer: "A", // 更新 userAnswer
        isCorrect: false // 更新 isCorrect
    };
    const savedEntry3_1 = await saveHistoryEntry(questionType3, entryData3_1);
    ...
    const history1 = await getHistory(questionType1);
    console.log(`[ManualTest] 題型 ${questionType1} 的歷史記錄 (應有 ${history1.length} 筆, 最新在前):`);
    history1.forEach((entry: HistoryEntry) => console.log(entry));

    console.log(`\n[ManualTest] 獲取題型 ${questionType2} 的所有歷史記錄...`);
    const history2 = await getHistory(questionType2);
    console.log(`[ManualTest] 題型 ${questionType2} 的歷史記錄 (應有 ${history2.length} 筆):`);
    history2.forEach((entry: HistoryEntry) => console.log(entry));
    
    console.log(`\n[ManualTest] 獲取題型 ${questionType3} 的所有歷史記錄...`);
    const history3 = await getHistory(questionType3);
    console.log(`[ManualTest] 題型 ${questionType3} 的歷史記錄 (應有 ${history3.length} 筆):`);
    history3.forEach((entry: HistoryEntry) => console.log(entry));

    console.log("\n[ManualTest] 嘗試獲取一個不存在題型的歷史記錄 (例如 '9.9.9')...");
    const historyNonExistent = await getHistory('9.9.9');
    ...
    if (history1.length >= 2) {
        ...
        const history1_limit1 = await getHistory(questionType1, 1);
        console.log(`[ManualTest] 結果 (應有 1 筆):`, history1_limit1);

        console.log("\n[ManualTest] 獲取從第 2 筆開始的 1 筆記錄 (limit: 1, offset: 1)...");
        const history1_offset1_limit1 = await getHistory(questionType1, 1, 1);
        ...
        const history1_offset_overflow = await getHistory(questionType1, 5, 10);
