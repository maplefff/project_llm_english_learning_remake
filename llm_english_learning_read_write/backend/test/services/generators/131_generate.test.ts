import { generate131Question } from '../../../src/services/generators/131_generate';
import { QuestionData131 } from '../../../src/services/generators/QuestionGeneratorInterface';

describe('131_generate', () => {
  test('應該生成有效的 1.3.1 段落主旨題', async () => {
    const result = await generate131Question(1);
    
    expect(result).not.toBeNull();
    expect(Array.isArray(result)).toBe(true);
    
    if (result && result.length > 0) {
      const question = result[0] as QuestionData131;
  ...
  test('應該生成多個題目', async () => {
    const result = await generate131Question(2);
  ...
  test('應該處理不同的難度設定', async () => {
    const easyResult = await generate131Question(1, "", 30);
    const hardResult = await generate131Question(1, "", 90);
  ...
  test('應該處理歷史摘要', async () => {
    ...
    const result = await generate131Question(1, historyMock);
  ...
  test('應該在生成失敗時返回 null', async () => {
    // 測試無效參數
    const result = await generate131Question(0);
