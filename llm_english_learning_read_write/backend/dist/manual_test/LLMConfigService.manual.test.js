...
async function runTest(testName, setup, verification) {
    console.log(`\n--- 執行測試: ${testName} ---`);
    try {
        setup();
...
async function main() {
    ...
    backupConfigFile();
    // --- 測試案例 --- 
    await runTest("配置文件不存在", () => { deleteConfigFile(); }, () => {
    ...
    await runTest("配置文件為空JSON", () => { writeConfigFile({}); }, () => {
        const config = LLMConfigService_1.LLMConfigService.getInstance().getConfig("any_id");
        assertConfig("any_id", defaultConfigValues, config);
    });
    await runTest("配置文件JSON格式錯誤", () => { fs.writeFileSync(CONFIG_FILE_PATH, "invalid json"); }, () => {
