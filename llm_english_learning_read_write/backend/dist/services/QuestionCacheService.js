...
class QuestionCacheService {
    ...
    // 佔位符方法，後續實現
    async _checkAndTriggerReplenishment(questionType) {
    ...
    // --- 背景補充任務 ---
    async _triggerReplenishment(questionType) {
        ...
        try {
            ...
            for (let attempt = 0; attempt <= MAX_GENERATION_RETRIES; attempt++) {
                try {
                    // 調用題目生成服務
                    const result = await QuestionGeneratorService_1.QuestionGeneratorService.generateQuestionByType(questionType, 70, // 預設難度，需要確認是否應從某處獲取
    ...
    // --- 1.1.2 背景補充任務 ---
    async _triggerReplenishment112(questionType) {
        if (this.isReplenishing.get(questionType)) {
            console.log(`[DEBUG QuestionCacheService.ts] Replenishment for ${questionType} is already in progress. Skipping.`);
            return;
        }
        ...
        try {
            ...
            for (let attempt = 0; attempt <= MAX_GENERATION_RETRIES; attempt++) {
                try {
                    const result = await QuestionGeneratorService_1.QuestionGeneratorService.generateQuestionByType(questionType, 70, "No history available.", needed);
                    if (result === null) {
                        newQuestionDataArray = null;
                    }
                    else if (Array.isArray(result)) {
    ...
    // --- 持久化快取到檔案 ---
    async _persistCacheToFile(questionType) {
    ...
    // --- 持久化 1.1.2 快取到檔案 ---
    async _persistCacheToFile112(questionType) {
        const questionsToSave = this.caches.get(questionType) ?? [];
    ...
    // 批量添加問題到快取
    async addQuestions(type, entries) {
        const cache = this.caches.get(type) ?? [];
        cache.push(...entries);
        await this._persistCacheToFile(type);
