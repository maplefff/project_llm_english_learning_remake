"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.generate281Question = generate281Question;
const LLMConfigService_1 = require("../../utils/LLMConfigService");
const GeminiAPIService_1 = __importDefault(require("../GeminiAPIService"));
const RateLimiter_1 = require("../../interfaces/RateLimiter");
const genai_1 = require("@google/genai");
const QUESTION_DATA_281_ITEM_SCHEMA = {
    type: genai_1.Type.OBJECT,
    properties: {
        source_text: { type: genai_1.Type.STRING },
        instruction: { type: genai_1.Type.STRING },
        reference_translations: {
            type: genai_1.Type.ARRAY,
            items: { type: genai_1.Type.STRING },
            minItems: 1,
            maxItems: 3
        },
        evaluation_focus: {
            type: genai_1.Type.ARRAY,
            items: { type: genai_1.Type.STRING },
            minItems: 2,
            maxItems: 4
        },
        difficulty_hint: { type: genai_1.Type.STRING }
    },
    required: ['source_text', 'instruction', 'reference_translations', 'evaluation_focus', 'difficulty_hint'],
};
const QUESTION_DATA_281_ARRAY_SCHEMA = {
    type: genai_1.Type.ARRAY,
    items: QUESTION_DATA_281_ITEM_SCHEMA,
};
async function generate281Question(numberOfQuestions = 1, historySummary = "", difficultySetting = 70) {
    console.log(`[DEBUG 281_generate.ts] Starting generation for ${numberOfQuestions} English-to-Chinese sentence translation questions, difficulty: ${difficultySetting}`);
    try {
        const prompt = `Generate ${numberOfQuestions} English-to-Chinese sentence translation questions. Return JSON array format.`;
        const config = LLMConfigService_1.LLMConfigService.getInstance().getConfig('2.8.1');
        const response = await GeminiAPIService_1.default.getResponse(prompt, {
            responseSchema: QUESTION_DATA_281_ARRAY_SCHEMA,
            config,
        }, RateLimiter_1.PRIORITY_LEVELS.LOW, 'generator_281');
        if (!Array.isArray(response)) {
            console.error('[DEBUG 281_generate.ts] Invalid response type from LLM');
            return null;
        }
        console.log(`[DEBUG 281_generate.ts] Generated ${response.length} question(s) for type 2.8.1.`);
        return response;
    }
    catch (error) {
        console.error('[DEBUG 281_generate.ts] Error during question generation:', error);
        return null;
    }
}
//# sourceMappingURL=281_generate.js.map