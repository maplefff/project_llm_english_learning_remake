...
describe('generate111Question', () => {
    ...
    it('成功生成指定數量的題目 (questionNumber > 1)', async () => {
        ...
        const result = await (0, _111_generate_1.generate111Question)(requestedNumber, "history", 70);
    ...
    it('當 LLM 回應不是陣列時應該拋出錯誤', async () => {
        ...
        await expect((0, _111_generate_1.generate111Question)(1, "", 70)).rejects.toThrow('從 LLM 收到的回應不是有效的陣列。');
    });
    it('當 LLM 回應的陣列中物件結構不符合 Zod Schema 時應該拋出錯誤', async () => {
        ...
        mockGetResponse.mockResolvedValue(invalidApiResponse);
        await expect((0, _111_generate_1.generate111Question)(1, "", 70)).rejects.toThrow(/LLM 回應陣列驗證失敗.*passage/);
    });
    it('當 LLM 回應的陣列中物件的 standard_answer 無效時應該拋出錯誤', async () => {
        ...
        mockGetResponse.mockResolvedValue(invalidApiResponse);
        await expect((0, _111_generate_1.generate111Question)(1, "", 70)).rejects.toThrow(/standard_answer 必須對應 options 陣列中某個元素的 id/);
    });
    it('當 LLM 回應的陣列為空時應該拋出 Zod 錯誤', async () => {
        const emptyApiResponse = []; // 空陣列
        mockGetResponse.mockResolvedValue(emptyApiResponse);
        await expect((0, _111_generate_1.generate111Question)(1, "", 70)).rejects.toThrow(/回應的陣列至少需要包含一個題目/); // 來自 .min(1)
    });
    it('當 Gemini API 服務拋出錯誤時應該重新拋出', async () => {
        ...
        await expect((0, _111_generate_1.generate111Question)(1, "", 70)).rejects.toThrow(errorMessage);
    });
    ...
    it('應該使用正確的 difficultySetting 構建 Prompt', async () => {
        ...
        mockGetResponse.mockResolvedValue(mockApiResponse);
        await (0, _111_generate_1.generate111Question)(requestedNumber, "", difficulty);
        expect(mockGetResponse).toHaveBeenCalledTimes(1);
    ...
    it('當 LLM 返回的題目數量與請求不符時，應記錄警告但仍返回數據', async () => {
        ...
        const result = await (0, _111_generate_1.generate111Question)(requestedNumber, "", 70);
        expect(mockGetResponse).toHaveBeenCalledTimes(1);
