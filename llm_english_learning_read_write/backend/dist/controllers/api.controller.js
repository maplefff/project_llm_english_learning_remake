...
class ApiController {
    ...
    async submitAnswer(req, res, next) {
        try {
            ...
            const result = await orchestratorService.submitAnswer(questionId, userAnswer, questionDataSnapshot);
            if (result) {
                res.json(result);
            }
            else {
    ...
    async getHistory(req, res, next) {
        try {
            ...
            const historyEntries = await (0, HistoryService_1.getHistory)(questionType, limitNum, offsetNum);
