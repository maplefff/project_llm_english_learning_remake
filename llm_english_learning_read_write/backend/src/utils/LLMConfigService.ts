...
// 合理範圍檢查
const isValidTemperature = (temp: any): temp is number => 
  typeof temp === 'number' && temp >= 0 && temp <= 2;

const isValidThinkingBudget = (budget: any): budget is number | null =>
  budget === null || (typeof budget === 'number' && budget >= 0 && budget <= 24576);


export class LLMConfigService {
  private static instance: LLMConfigService;
  private configs: GeneratorConfigs = {};
  ...
  private loadConfig(): void {
    try {
      ...
      this.configs = JSON.parse(fileContent) as GeneratorConfigs;
  ...
  public getConfig(generatorId: string): FullLLMConfig {
