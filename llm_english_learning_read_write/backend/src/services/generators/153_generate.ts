import { QuestionData153 } from './QuestionGeneratorInterface';
 ...
 */
export async function generate153Question(
    numberOfQuestions: number = 1,
    historySummary: string = "",
    difficultySetting: number = 70
): Promise<QuestionData153[] | null> {
...
Return ONLY the JSON array. No markdown formatting.`;
        ...
        // 取得 LLM 設定
        const config = LLMConfigService.getInstance().getConfig('1.5.3');
        ...
        const response = await GeminiAPIService.getResponse(prompt, {
            ...
            config,
        }, PRIORITY_LEVELS.MEDIUM, 'generator_153');
        ...
        return response as QuestionData153[];
