...
// 從共享介面導入類型
import { QuestionData112 } from './QuestionGeneratorInterface';
 ...
 */
export async function generate112Question(
    questionNumber: number = 1,
    historySummary: string = "", 
    difficultySetting: number = 70 
): Promise<QuestionData112[] | null> {

    const PROMPT_TEMPLATE_1_1_2 = 
...
`;
    ...
    // 取得 LLM 設定
    const config = LLMConfigService.getInstance().getConfig('1.1.2');
    ...
    try {
        ...
        if (response.length !== questionNumber) {
            console.warn(
                `[DEBUG 112_generate.ts] LLM returned ${response.length} questions, but ${questionNumber} were requested. Proceeding with received data.`
            );
        }
        
        // 假設 Gemini 的 Schema 功能已確保內部結構基本正確
        // 如果需要更嚴格的運行時驗證，可以考慮後續添加 Zod 或類似庫
        return response as QuestionData112[]; // 返回數據陣列
