...
class RateLimiterService {
  ...
  private requestTimestamps: number[] = []; // 記錄每個請求的時間戳
  ...
  async enqueueRequest(
    ...
    return new Promise((resolve, reject) => {
      ...
      // 緊急模式：只允許高優先權請求
      if (this.config.emergencyMode && priority !== PRIORITY_LEVELS.HIGH) {
   ...
   */
  private async makeActualAPICall(prompt: string, options?: any): Promise<any> {
    // 暫時的模擬實現
  ...
  public getStatus(): RateLimitStatus {
    ...
    const recentMetrics = this.metrics.filter(m => 
      m.timestamp.getTime() > Date.now() - this.config.windowSizeMs
    );
   ...
   * 切換緊急模式
   */
  public toggleEmergencyMode(enabled: boolean): void {
    this.config.emergencyMode = enabled;
    console.log(`[DEBUG RateLimiterService.ts] 緊急模式已${enabled ? '啟用' : '停用'}`);
    
    if (enabled) {
      // 清空所有非高優先權請求
      const lowPriorityRequests = this.requestQueue.filter(req => req.priority !== PRIORITY_LEVELS.HIGH);
      lowPriorityRequests.forEach(request => {
        request.promise.reject(new Error('緊急模式啟用，非高優先權請求已取消'));
      });
      this.requestQueue = this.requestQueue.filter(req => req.priority === PRIORITY_LEVELS.HIGH);
