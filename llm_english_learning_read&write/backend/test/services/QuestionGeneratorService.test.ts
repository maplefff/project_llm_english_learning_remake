...
// 導入類型用於定義 mock 回應
import { QuestionData111 } from '../../src/services/generators/111_generate';
...
const mockGenerate111Question = generate111Question as jest.MockedFunction<typeof generate111Question>;

// 定義一個用於測試的模擬 QuestionData111 物件
const mockQuestionData: QuestionData111 = {
...
describe('QuestionGeneratorService', () => {
  ...
  it('應該成功為類型 1.1.1 調用 generate111Question 並返回結果', async () => {
    ...
    // 行動: 調用服務函數
    const result = await QuestionGeneratorService.generateQuestionByType(questionType, difficulty, history);
  ...
  it('當 generate111Question 返回 null 時應該返回 null', async () => {
  ...
  it('當 generate111Question 拋出錯誤時應該捕獲錯誤並返回 null', async () => {
    // 安排: 讓模擬函數在被調用時拋出錯誤
